<section class="section-intro pt-80 pb-24 px-8 xl:px-0">
	<div class="xl:container mx-auto grid grid-cols-1 lg:grid-cols-12">
		<div class="title col-span-1 lg:col-span-3 flex flex-col lg:block">
			<h1 class="text-5xl text-main-green font-extrabold mb-8 lg:mb-12 order-2 lg:order-1"><?php the_archive_title(); ?></h1>
			<p class="flex items-center text-sm text-main-green font-semibold tracking-wider uppercase order-1 lg:order-2 mb-8 lg:mb-0"><span class="block w-20 h-px bg-main-green mr-5"></span><?php the_archive_title(); ?></p>
		</div>
		<div class="description col-span-1 lg:col-span-4">
			<p class="text-sm text-main-green font-light leading-8 tracking-wider">
				Lorem ipsum, dolor sit amet consectetur adipisicing elit. Assumenda excepturi eaque iure odit dolorem nostrum tempore dolore sint totam explicabo. Eveniet cum officiis perspiciatis nesciunt cumque voluptatum quo, impedit aliquam?
			</p>
		</div>
		<div class="description col-span-1 lg:col-span-4"">
			<p class="text-sm text-main-green font-light leading-8 tracking-wider">
				Lorem ipsum, dolor sit amet consectetur adipisicing elit. Assumenda excepturi eaque iure odit dolorem nostrum tempore dolore sint totam explicabo. Eveniet cum officiis perspiciatis nesciunt cumque voluptatum quo, impedit aliquam?
			</p>
		</div>
	</div>
</section>
