...
if ( $images ) : ?>
	<section class="section-gallery lg:pt-[120px]">
		...
		<div thumbsSlider="" class="swiper zimmer-thumb-swiper w-full max-auto h-[20vh] hidden lg:block">
			<div class="swiper-wrapper">
				<?php foreach ( $images as $image ) : ?>
					<div class="swiper-slide hover:cursor-pointer">
						<img class="block w-full h-full max-auto object-cover" src="<?php echo esc_url( $image['sizes']['zimmer-gallery-thumb'] ); ?>" alt="Thumbnail of <?php echo esc_url( $image['alt'] ); ?>" />
					</div>
				<?php endforeach; ?>
			</div>
		</div>
